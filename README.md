...
> 你有一个需求，打算从头写。
> 你不知道的是：世界上已经有人把它做成了 skill，开源放在那里——
> 可能藏在一个合集仓库的三层目录下；
> 可能有八个一模一样的拷贝挤在搜索结果里，其中一个还把原作者的署名抹了；
> 也可能有八种各自不同的实现，等你挑一个最对口的。
>
...
| 搜到的候选真假难辨 | 修谱认出谁是原版、谁是镜像、谁被剥了署名——把功劳还给原作者，把对的版本装给你 |
...
这条管线最早不是为开源做的，是自用流程的固化：每次有新需求，先让世界帮一把，找不到再自己写。[cases/](./cases/) 里的两篇就是从很多次实际使用里挑出来的典型——一次撞见市场拷贝抹掉了 Microsoft 的署名，一次同样八条结果却是八种不同实现。**说白了：见过一次"搜索第一名是抄的第二名"，你就不会再裸装搜索结果了。**
...
> 两篇是从大量实际使用里挑出来的典型，不是全部；新的典型会持续补充。

| 案例 | 一句话剧透 |
|---|---|
| [被抹掉署名的 skill](./cases/01-the-stripped-credit.md) | 搜索第一名（349⭐）是第二名的镜像，唯二改动是删掉 `license: MIT` 和 `author: Microsoft`——管线把原版装了回来 |
| [八条结果八个样](./cases/02-eight-results-eight-kinds.md) | 同样八条结果：一次是八胞胎拷贝、一次是八种独立实现——归族认出地貌，分支才走得对 |
